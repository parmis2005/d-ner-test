    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
        <a href="#top" className="flex items-center gap-2.5 group">
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-fire shadow-md shadow-fire/40 transition-transform group-hover:scale-105">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-sm font-medium tracking-wide text-cream/80 transition-colors hover:text-fire-light"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="hidden items-center gap-4 lg:flex">
          <a
            ...
            className="flex items-center gap-2 text-sm font-semibold text-cream/90 hover:text-fire-light transition-colors"
          >
          ...
          <a
            href="#bestellen"
            className="rounded-full bg-fire px-5 py-2.5 text-sm font-bold uppercase tracking-wide text-cream shadow-lg shadow-fire/30 transition-all hover:bg-fire-light hover:shadow-fire-light/40"
          >
            Jetzt bestellen
          </a>
        </div>
      ...
      {open && (
        <div className="flex flex-col gap-1 border-t border-white/10 bg-charcoal px-5 pb-6 pt-2 lg:hidden">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
